...
function xmldb_widget_upgrade($oldversion) {
    ...
    if ($oldversion < 201604240210) {

        // Insert code here to perform some actions (same as in install.php).
	// Define field livestarttime to be added to widget.
        $table = new xmldb_table('widget');
        $field = new xmldb_field('livestarttime', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'grade');

        // Conditionally launch add field livestarttime.
        if (!$dbman->field_exists($table, $field)) {
            $dbman->add_field($table, $field);
        }


        upgrade_mod_savepoint(true, 201604240210, 'widget');
    }
